...
#Plot specifications
jpeg(file = "SNP_data_10KbTo1Mb.jpg", width=1200*3, height=500*11/2, pointsize = 16)
par(mfrow=c(8,3))
...
#plot specifications
jpeg(file = "ROH_data_10KbTo1Mb.jpg", width=1200*3, height=500*11/2, pointsize = 16)
par(mfrow=c(8,3))
par(mar=c(5,7,2,2)+0.1)

#Grab files 
fn = dir(pattern = "PermFormat_btwnTenKbandOneMbROH_", full.names = TRUE)
fn = str_sort(fn, numeric = TRUE) 
...
fn = dir(pattern = "_FinalVEPAnnots.txt", full.names = TRUE)
fn = str_sort(fn, numeric = TRUE) 
...
for (c in fn) {
  ...
  temp = temp %>% mutate(chr = as.factor(temp$chrNum)) %>%
    semi_join(ROH_data_pos, by=c("chr","pos")) %>% select(-c(chr)) %>%
...
# Save all data
save(snp_data, ROH_data, annotation_data, group, file="all_SNP_10KbTo1Mb_annotation_DogProjectClareDec2018.Rdata")
