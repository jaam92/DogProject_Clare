...
setwd("/u/home/j/jmooney3/klohmueldata/jazlyn_data/DogProject_Clare/ComputeG12")
...
for (i in pops){
 ...
 indivs = read.table(file = paste("/u/home/j/jmooney3/klohmueldata/jazlyn_data/DogProject_Clare/IndivFiles/" ,i, "_indivList_downsample_N6.txt", sep = "")) 
  ...
  for (chrom in 1:38){
  	vcf = fread(file=paste("/u/home/j/jmooney3/klohmueldata/jazlyn_data/DogProject_Clare/RecodedVCF/AssignGT_WildDogasREF_reformatted_masterFile_allCanidsN75_ANmt135_Chr", chrom, ".vcf", sep=""),check.names=F) %>% select(-c(V1)) #remove the null column that get's added
