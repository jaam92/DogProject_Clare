...
#source(file = "generateRdataFile4Permuation_btwnTenKbandOneMb.R")

#Load data
load(file="/u/home/j/jmooney3/klohmueldata/jazlyn_data/DogProject_Clare/ROHPermutation/all_SNP_10KbTo1Mb_annotation_DogProjectClareDec2018.Rdata")
...
calc_contingency_table = function(snpmat, rohmat, annotation, removeFixed=TRUE) {
  ...
  rowIDsyn = annotation == "SY"
  rowIDnonsyn = annotation == "NS"
  ...
  table_alleleCopies = matrix(c(countDerived_SynROH_alleleCopies, countDerived_NonsynROH_alleleCopies, countDerived_SynNONROH_alleleCopies, countDerived_NonsynNONROH_alleleCopies), nrow=2, dimnames = list(c("Syn", "Nonsyn"), c("ROH", "nonROH")))
  table_variants = matrix(c(countDerived_SynROH_variants, countDerived_NonsynROH_variants, countDerived_SynNONROH_variants, countDerived_NonsynNONROH_variants), nrow=2, dimnames = list(c("Syn", "Nonsyn"), c("ROH", "nonROH")))
  table_homozygotes = matrix(c(countDerived_SynROH_homozygotes, countDerived_NonsynROH_homozygotes, countDerived_SynNONROH_homozygotes, countDerived_NonsynNONROH_homozygotes), nrow=2, dimnames = list(c("Syn", "Nonsyn"), c("ROH", "nonROH")))
...
if(FALSE) {
  jpeg("permutation_10KbTo1Mb.jpg", width=1200, height=1500/4*5)
  ...
  snp_data_ann[annotation_data$annotation=="NS",] = snp_data_ann[annotation_data$annotation=="NS",] + 4
...
save(file="permutation_results_allCanids_btwnTenKbandOneMb.Rdata", pval_group, X_squared_group, empirical_table_group, ratios_group)



#Plots

load(file="permutation_results_allCanids_btwnTenKbandOneMb.Rdata")
...
write.csv(ratios_group_df, row.names = FALSE, file = "ratios_pvalues_pergroup_btwnTenKbandOneMb.csv")
