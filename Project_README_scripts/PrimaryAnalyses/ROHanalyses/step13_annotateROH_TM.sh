##qsub -cwd -V -N ROHQual -l highp,time=10:00:00,h_data=5G -t 1-38:1 -M eplau -m ea annotateROH_TM.sh
...
#Run script
        ...
        ROHrange_in=${in_dir}/'TM_Chr'$CHROM'_ROHRanges.txt'

        sites_in=${site_dir}/'TenTM_chr'$CHROM'_allSitesPosOnly.txt'

        count_out=${out_dir}/'TM_Chr'$CHROM'_goodSitesWithinROH.txt'
