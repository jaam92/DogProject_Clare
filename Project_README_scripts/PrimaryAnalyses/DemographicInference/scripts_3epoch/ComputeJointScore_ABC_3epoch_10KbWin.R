...
option_list <- list( 
  make_option(c("--LengthThreshold"), type="integer", default=2000, 
  ...
  make_option(c("--EmpInfile"), type="character", default="/u/home/j/jmooney3/klohmueldata/jazlyn_data/DogProject_Clare/RunABC/InputData/EWSamps_allChroms_NeutralRegions_het_10000win_10000step_ABCinputFile.txt", 
...
simulationParameterFile = read.table(file=opt$paramFile, col.names = c("NeCurr","Ne2","Tbot1","Tbot2","NeAnc"))
...
createSumStatDF <- function(dataFrame, colInterest, summaryID){
  dataFrame %>%
    ...
    spread(bin, colInterest) %>% #make data long 
    select(-c(row_id)) %>% #remove the row id
    mutate(SumStat = summaryID) %>% #name the summary stat we are tracking
    cbind.data.frame(simulationParameterFile, TrackSumStats) %>% #add on sim params
    select(SumStat, NeCurr, Ne2, NeAnc, Tbot1, Tbot2, jointScore, SegSiteScore, PiScore, everything())
}
...
#find errors with bins
Duplicates = table(duplicated(mergedPIandSegSites$bin))
print("Duplicates, should all be false")
print(Duplicates)
