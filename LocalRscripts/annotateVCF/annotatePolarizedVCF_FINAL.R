...
for (chrom in 1:38){
  ...
  #Load file
  annotation = read.table(file=paste("~/Documents/DogProject_Clare/LocalRscripts/parseVEP/parsedVEP_chr", chrom, "_codingAnnots.txt", sep=""))
  head(annotation)
  ...
  indivs = colnames(annotatedVCF[,5:79])
    ...
    for (i in indivs){
      ...
      indivID = gsub(":GT", "", i)
      write.table(indivAnnotDF, file=paste("annotatedGTwithVEP_",indivID, "_Chr" ,chrom, ".txt", sep=""), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
    }
