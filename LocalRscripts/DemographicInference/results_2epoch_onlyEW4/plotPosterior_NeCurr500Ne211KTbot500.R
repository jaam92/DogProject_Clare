...
set.seed(1)
samps = seq(1:100000)
Ne1 = runif(samps, min = 500, max = 10000)
Ne2 = runif(samps, min = 11000, max = 40000)
Tbot = runif(samps, min = 500, max = 3000)
sampPriors = cbind.data.frame(samps,Ne1,Ne2,Tbot)
plotNe1 = ggplot() + geom_density(aes(x=Ne1), colour="blue") + ggtitle(expression(paste(Ne[1]))) + xlab("unif(500,10000)") + theme_bw()+ theme(axis.text.x = element_text(size  = 20), axis.text.y = element_text(size = 20), axis.title=element_text(size=20), plot.title = element_text(size=20, hjust = 0.5),legend.title=element_blank(), legend.text=element_text(size=18),legend.position="bottom")
...
plotTbot = ggplot() + geom_density(aes(x=Tbot), colour="green") + ggtitle(expression(paste(T[bottleneck]))) + xlab("unif(500,300)") + theme_bw()+ theme(axis.text.x = element_text(size  = 20), axis.text.y = element_text(size = 20), axis.title=element_text(size=20), plot.title = element_text(size=20, hjust = 0.5),legend.title=element_blank(), legend.text=element_text(size=18),legend.position="bottom")

plotPriorsTogether = plot_grid(plotNe1,plotNe2+ylab(""),plotTbot+ylab(""),nrow = 1, align = 'h')


#Load ABC results
setwd("~/Documents/DogProject_Clare/LocalRscripts/DemographicInference/ABCResults_NeCurr500to10kNeAnc11kto40kT500to3000/")
